...
func (r *RepodataResolver) loadPrimary(primURL string, baseurl string) ([]entry, error) {
	...
	defer resp.Body.Close()
	if resp.StatusCode != 200 {
		return nil, fmt.Errorf("%s: %s", primURL, resp.Status)
	}
	...
	defer zr.Close()
	var meta primary
	if err := xml.NewDecoder(zr).Decode(&meta); err != nil {
		return nil, err
	}
...
func evrString(n nevr) string {
	if n.epoch > 0 {
		return fmt.Sprintf("%d:%s-%s", n.epoch, n.ver, n.rel)
	}
